 ...
 */
export function countTimeLog(
	timeLabelsInfo: string[] | Record<string, string>,
	totalTimeLabel = '',
) {
	if (!isAllow) {
		return false
	...
	const timetLabels: string[] = Array.isArray(timeLabelsInfo)
		? timeLabelsInfo
		: Object.keys(timeLabelsInfo)

	// [total time] 总耗时
	let totalTime = 0
	let totalTimes = totalTimeLabel && getTimeDiffs(totalTimeLabel)
	if (totalTimes.length == 0) {
		timetLabels.forEach((label) => {
